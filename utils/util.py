...
def visualize(annotations, category_id_to_name):
    ...
    '''
    img = annotations[0]['image'].squeeze().numpy().transpose(1,2,0).copy()
    mean = np.array([0.3442, 0.3708, 0.3476])
    std = np.array([0.1232, 0.1230, 0.1284])
    img = std * img + mean
    img = np.clip(img, 0, 1)
    img = (img*255).astype(np.uint8)
    annotations[1][0]['labels'] = annotations[1][0]['labels']
    length = len(annotations[1][0]['labels'])
    if length == 1:
        img = visualize_bbox(img, annotations[1][0]['boxes'][0].squeeze().numpy(), int(annotations[1][0]['labels'].squeeze()), category_id_to_name)
    else:
        for idx, bbox in enumerate(annotations[1][0]['boxes'].numpy()):
            img = visualize_bbox(img, bbox, int(annotations[1][0]['labels'][idx]), category_id_to_name)
